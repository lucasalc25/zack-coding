...
export default class NovoJogo extends Phaser.Scene {
    constructor() {
        super({ key: 'NovoJogo' });
        this.bgImage;
        this.character;
    ...
    create() {
        ...
        if (this.registry.get('musicOn')) {
            this.musicGameplay = this.sound.add('musicGameplay', { loop: true });
            this.musicGameplay.setVolume(localStorage.getItem("musicVolume") * 0.7);
            this.musicGameplay.play();
        }
    ...
    shutdown() {
        this.musicGameplay.stop(); // Para o áudio ao sair da cena
    }
    ...
    showChoices() {
        this.textTitle = this.add.text(this.game.canvas.width * 0.5, this.game.canvas.height * 0.7, 'Clique no seu nível:', { fontFamily: 'Arial', fontSize: '24px', fill: '#fff' }).setOrigin(0.5).setWordWrapWidth(this.game.canvas.width * 0.9);
        // Cria botões de escolha
        this.basic = this.add.text(this.game.canvas.width * 0.2, this.game.canvas.height * 0.8, 'Básico', { fontFamily: 'Arial', fontSize: '20px', fill: '#fff', padding: 15, fontWeight: 'bold' }).setOrigin(0.5);
        this.intermediary = this.add.text(this.game.canvas.width * 0.5, this.game.canvas.height * 0.8, 'Intermediário', { fontFamily: 'Arial', fontSize: '20px', fill: '#666', padding: 15, fontWeight: 'bold' }).setOrigin(0.5);
        this.advanced = this.add.text(this.game.canvas.width * 0.8, this.game.canvas.height * 0.8, 'Avançado', { fontFamily: 'Arial', fontSize: '20px', fill: '#666', padding: 15, fontWeight: 'bold' }).setOrigin(0.5);
                ...
                this.typeText(this.dialogueText, `Ok, começaremos do ${localStorage.getItem("nivel")} então. Vamos lá!`, 0, () => {
                    ...
                    this.input.once('pointerdown', () => {
                        ...
                        setTimeout(() => {
                            ...
                            this.scene.stop('NovoJogo');
                            if (this.nivel === 'Básico') this.scene.start('Jogo');
