...
export default class Jogo extends Phaser.Scene {
    ...
    init(data) {
        this.phaseIndex = data.faseAtual || 0; // Define a fase inicial como 0 se não for fornecida
    ...
    update() {
        ...
        if (this.musicOn == false) {
            this.musicGameplay = this.sound.add('musicGameplay', { loop: true });
            this.musicGameplay.setVolume(localStorage.getItem("musicVolume"));
            this.musicGameplay.play();
        }
    ...
    resetGame() {
        ...
        if (localStorage.getItem("nivel") === 'Básico') {
            this.scene.stop('Jogo'); // Encerre a cena atual e retorne ao menu principal
            this.scene.start('Loading', { nextScene: MainMenu, faseAtual: this.faseAtual });
        }
        /* if(localStorage.getItem("nivel") === 'Intermediário') {
            ...
            this.scene.start('Load2', { faseAtual: this.faseAtual });
        } */
